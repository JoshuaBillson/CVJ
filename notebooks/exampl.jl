...
# ╔═╡ c59891d0-99e1-11ed-181f-9b671426f427
using Plots, Images, FileIO

# ╔═╡ 0c946692-6499-4772-8365-4881e0d4521c
using CategoricalArrays

# ╔═╡ d199e164-7e2c-42e3-ada9-451775915265
using Random
...
begin
	...
	function DataPipeline(
		...
		l::AbstractVector{L}, 
		size::Tuple{Int, Int}; 
		aug=Augmentor.ImmutablePipeline(NoOp())) where {I <:AbstractImage, L}
		DataPipeline(i, l, size, aug)
	end
end;

# ╔═╡ 2103453e-504e-4576-9bfc-fa778c4361d0
...
struct Machine{I<:AbstractImage, T, M<:AbstractModel}
	model::M
	X::AbstractVector{I}
	Y::AbstractVector{T}
...
function fit!(
	machine::Machine{I, T, M}, 
	train::AbstractVector{Int}, 
	test::AbstractVector{Int}, 
	val::AbstractVector{Int}) where {I<:AbstractImage, T, M<:AbstractModel}
	...
	Y_train = machine.Y[train]
	train_pipeline = DataPipeline(X_train, Y_train, sz, aug)
	train_data = Flux.DataLoader(train_pipeline, batchsize=32, shuffle=true)

	# Get Test Data
	X_test = machine.X[test]
	Y_test = machine.Y[test]
	test_pipeline = DataPipeline(X_test, Y_test, sz)
	test_data = Flux.DataLoader(test_pipeline, batchsize=32, shuffle=true)
